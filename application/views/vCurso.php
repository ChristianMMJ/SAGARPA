<!--NAVEGACION INTERNA-->
<div id="pnlNavegacionSesiones" class="animated fadeInLeft">  
    <div class="col-md-12 dt-buttons" align="right">
        <a href="<?php print base_url(); ?>" type="button" class="btn btn-default">
            <span class="fa fa-home fa-1x"></span><br>IR AL INICIO
        </a>
    </div>
    <div class="col-md-12"><center><h3>MÓDULO 1</h3></center></div>
        <div class="col-md-4 disabledDetalle hide" id="1"> 
            <div class="thumbnail">
                <img src="<?php print base_url(); ?>img/bienvenida.jpg" alt="Bienvenida thumbnail" class="img-responsive"> 
                <div class="caption"> 
                    <h3>SESIÓN 1</h3>
                    <p>Oportunidades del sector agroalimentario mexicano
                    </p>
                    <hr>
                    <a href="<?php print base_url('CtrlContenidoSesionCursoUno') ?>" type="button" class="btn btn-raised btn-primary pull-right">INICIAR</a>
                    <br>
                    <br>
                    </p> 
                </div>
            </div>
        </div>
        <div class="col-md-4 disabledDetalle hide" id="2"> 
            <div class="thumbnail">
                <img src="<?php print base_url(); ?>img/bienvenida.jpg" alt="Bienvenida thumbnail" class="img-responsive"> 
                <div class="caption"> 
                    <h3>SESIÓN 2</h3>
                    <p>Apoyos y subsidios para el sector agroalimentario en México
                    </p>
                    <hr>
                    <a href="<?php print base_url('CtrlContenidoSesionCursoDos') ?>" type="button" class="btn btn-raised btn-primary pull-right">INICIAR</a>
                    <br>
                    <br>
                    </p> 
                </div>
            </div>
        </div>
        <div class="col-md-4 disabledDetalle hide" id="3"> 
            <div class="thumbnail">
                <img src="<?php print base_url(); ?>img/bienvenida.jpg" alt="Bienvenida thumbnail" class="img-responsive"> 
                <div class="caption"> 
                    <h3>SESIÓN 3</h3>
                    <p>Creación de un negocio agroalimentario
                    </p>
                    <hr>
                    <a href="<?php print base_url('CtrlContenidoSesionCursoTres') ?>" type="button" class="btn btn-raised btn-primary pull-right">INICIAR</a>
                    <br>
                    <br>
                    </p> 
                </div>
            </div>
        </div>
    
        <div class="col-md-4 disabledDetalle hide" id="4"> 
            <div class="thumbnail">
                <img src="<?php print base_url(); ?>img/bienvenida.jpg" alt="Bienvenida thumbnail" class="img-responsive"> 
                <div class="caption"> 
                    <h3>SESIÓN 4</h3>
                    <p>Producción agroalimentarios con Valor Agregado
                    </p>
                    <hr>
                    <a href="<?php print base_url('CtrlContenidoSesionCursoCuatro') ?>" type="button" class="btn btn-raised btn-primary pull-right">INICIAR</a>
                    <br>
                    <br>
                    </p> 
                </div>
            </div>
        </div>
        <div class="col-md-4 disabledDetalle hide" id="5"> 
            <div class="thumbnail">
                <img src="<?php print base_url(); ?>img/bienvenida.jpg" alt="Bienvenida thumbnail" class="img-responsive"> 
                <div class="caption"> 
                    <h3>SESIÓN 5</h3>
                    <p>Mercado y cadena de suministro en empresas agroalimentarias
                    </p>
                    <hr>
                    <a href="<?php print base_url('CtrlContenidoSesionCursoCinco') ?>" type="button" class="btn btn-raised btn-primary pull-right">INICIAR</a>
                    <br>
                    <br>
                    </p> 
                </div>
            </div>
        </div>
        <div class="col-md-4 disabledDetalle hide" id="6"> 
            <div class="thumbnail">
                <img src="<?php print base_url(); ?>img/bienvenida.jpg" alt="Bienvenida thumbnail" class="img-responsive"> 
                <div class="caption"> 
                    <h3>SESIÓN 6</h3>
                    <p>Formulación del plan de negocio 
                    </p>
                    <hr>
                    <a href="<?php print base_url('CtrlContenidoSesionCursoSeis') ?>" type="button" class="btn btn-raised btn-primary pull-right">INICIAR</a>
                    <br>
                    <br>
                    </p> 
                </div>
            </div>
        </div>
    
        <div class="col-md-4 disabledDetalle hide" id="7"> 
            <div class="thumbnail">
                <img src="<?php print base_url(); ?>img/bienvenida.jpg" alt="Bienvenida thumbnail" class="img-responsive"> 
                <div class="caption"> 
                    <h3>SESIÓN 7</h3>
                    <p>Tecnologías y métodos de producción modernos
                    </p>
                    <hr>
                    <a href="<?php print base_url('CtrlContenidoSesionCurso1') ?>" type="button" class="btn btn-raised btn-primary pull-right">INICIAR</a>
                    <br>
                    <br>
                    </p> 
                </div>
            </div>
        </div>
        <div class="col-md-4 disabledDetalle hide" id="8"> 
            <div class="thumbnail">
                <img src="<?php print base_url(); ?>img/bienvenida.jpg" alt="Bienvenida thumbnail" class="img-responsive"> 
                <div class="caption"> 
                    <h3>SESIÓN 8</h3>
                    <p>Costos, inversiones y rentabilidad del negocio 
                    </p>
                    <hr>
                    <a href="<?php print base_url('CtrlContenidoSesionCursoOcho') ?>" type="button" class="btn btn-raised btn-primary pull-right">INICIAR</a>
                    <br>
                    <br>
                    </p> 
                </div>
            </div>
        </div>
        <div class="col-md-4 disabledDetalle hide" id="9"> 
            <div class="thumbnail">
                <img src="<?php print base_url(); ?>img/bienvenida.jpg" alt="Bienvenida thumbnail" class="img-responsive"> 
                <div class="caption"> 
                    <h3>SESIÓN 9</h3>
                    <p>Normas de inocuidad agroalimentaria
                    </p>
                    <hr>
                    <a href="<?php print base_url('CtrlContenidoSesionCursoNueve') ?>" type="button" class="btn btn-raised btn-primary pull-right">INICIAR</a>
                    <br>
                    <br>
                    </p> 
                </div>
            </div>
        </div>
    
        <div class="col-md-4 disabledDetalle hide" id="10"> 
            <div class="thumbnail">
                <img src="<?php print base_url(); ?>img/bienvenida.jpg" alt="Bienvenida thumbnail" class="img-responsive"> 
                <div class="caption"> 
                    <h3>SESIÓN 10</h3>
                    <p>Estrategias de comercialización de productos agroalimentarios
                    </p>
                    <hr>
                    <a href="<?php print base_url('CtrlContenidoSesionCursoDiez') ?>" type="button" class="btn btn-raised btn-primary pull-right">INICIAR</a>
                    <br>
                    <br>
                    </p> 
                </div>
            </div>
        </div>
        <div class="col-md-4 disabledDetalle hide" id="11"> 
            <div class="thumbnail">
                <img src="<?php print base_url(); ?>img/bienvenida.jpg" alt="Bienvenida thumbnail" class="img-responsive"> 
                <div class="caption"> 
                    <h3>SESIÓN 11</h3>
                    <p>Integración de cadenas productivas y comerciales
                    </p>
                    <hr>
                    <a href="<?php print base_url('CtrlContenidoSesionCursoOnce') ?>" type="button" class="btn btn-raised btn-primary pull-right">INICIAR</a>
                    <br>
                    <br>
                    </p> 
                </div>
            </div>
        </div>
        <div class="col-md-4 disabledDetalle hide" id="12"> 
            <div class="thumbnail">
                <img src="<?php print base_url(); ?>img/bienvenida.jpg" alt="Bienvenida thumbnail" class="img-responsive"> 
                <div class="caption"> 
                    <h3>SESIÓN 12</h3>
                    <p>Empresas agroalimentarias de exportación
                    </p>
                    <P>
                        Mercadotecnia para productos agroalimentarios
                    </P>
                    <hr>
                    <a href="<?php print base_url('CtrlContenidoSesionCurso1') ?>" type="button" class="btn btn-raised btn-primary pull-right">INICIAR</a>
                    <br>
                    <br>
                    </p> 
                </div>
            </div>
        </div>
    
        <div class="col-md-4 disabledDetalle hide" id="13"> 
            <div class="thumbnail">
                <img src="<?php print base_url(); ?>img/bienvenida.jpg" alt="Bienvenida thumbnail" class="img-responsive"> 
                <div class="caption"> 
                    <h3>CIERRE DEL CURSO</h3>
                    <p>Agradecimiento
                    </p>
                    <P>
                        Mensaje de motivación a emprender un negocio agroalimentario de calidad
                    </P>
                    <P>
                        Emisión de constancia
                    </P>

                    <hr>
                    <a href="<?php print base_url('CtrlContenidoSesionFinal') ?>" type="button" class="btn btn-raised btn-primary pull-right">INICIAR</a>
                    <br>
                    <br>
                    </p> 
                </div>
            </div>
        </div>
    
</div>

<script>
    var master_url = base_url + 'CtrlCurso/';
    var Nsesion = getCookie("sesion");
    var pnlNavegacionSesiones = $('#pnlNavegacionSesiones');
    var idUsuario = "<?php echo $this->session->userdata('ID'); ?>";

    $(document).ready(function () {
//        onNotify('', 'MENÚ DE CONTENIDOS', 'info');
//        for (var i = 1; i <= Nsesion; i++) { 
//             pnlNavegacionSesiones.find('#' + i).removeClass('disabledDetalle hide');
//        }
        getSesionesActivas();

    });

    function getSesionesActivas() {
        HoldOn.open({
            theme: "sk-bounce",
            message: "CARGANDO DATOS..."
        });
        $.ajax({
            url: master_url + 'getSesionesActivasByUsuario',
            type: "POST",
            dataType: "JSON",
            data: {
                ID: idUsuario
            }
        }).done(function (data, x, jq) {
            $.each(data, function (index, item) {
                pnlNavegacionSesiones.find('#' + item.NSesion).removeClass('disabledDetalle hide');
            });
        }).fail(function (x, y, z) {
            console.log(x, y, z);
        }).always(function () {
            HoldOn.close();
        });
    }
</script>
